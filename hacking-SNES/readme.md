...
3) [My favourite SNES games circa 1997](#links-to-some-of-the-most-popular-snes-roms-on-myrient)
...
Scanmem also has an affiliated GUI based tool, GameConqueror, but I've never used it so I can't speak for its effectiveness.
...
Computers operate in the same manner. Data stored on the hard disk cannot be used to perform a dynamic action. You cannot execute data stored on disk, in the same manner in which we cannot drive a car by only employing a memory of driving a car in the past. We need to physically operate the vehicle, which requires our minds to think about what we are doing actively. I.e. Remember stepping on a brake pedal won't prevent you from rear ending someone stopped in front of you.
...
Albeit largely a topic for science fiction (or Neuralink) for humans, rewriting a computer's memory is a trivial task. If we can locate a value we are interested in within the computer's memory, we can simply put a new value in its place.
...
To locate a running process on Linux we can use `ps`. We can pipe the results of `ps` to `grep` and immediately figure out the Snes9x process identification number (PID).
...
**With the PID for snes9x-gtk in-hand, 5703, we are ready to start hunting for values we want to change.**
...
The difficulty of isolating the value in memory is proportional to the length of known values in bits. For example, finding the value 532,128 would be significantly easier than 1.
...
3) [Super Fast Levelling in Chrono Trigger](#super-fast-levelling-in-chrono-trigger)
...
**Note:** If you only provide the integer 6, you are telling scanmem to match patterns of bits matching 100. As discussed [previously](#differential-analysis-as-it-applies-to-hacking-games), this will return a significantly greater number of matches than 006.
...
10) Input the new value for NEXT into scanmem:
	...
	- The variable is an integer that is larger than 8 bits (easy).
...
3) Wax & repeat until the surface coat is shiny without blemishes.
...
- If you close & reopen Snes9x, the process will be allotted a new space in the computer's memory.
...
In the following example from hacking Shadowrun, I have identified the address containing the value associated with Armitage's hit points. I use scanmem's write command to set a value for the hit points, rather than using the set command, which would change the value for all 3 matches:
...
SNES is full of games that have bars representing a player's health or magic points pool (hp/mp bars). These would seem obvious targets for the player to hack. I noted that Game Genie didn't contain a hack for Zelda's magic bar, however I was able to change it at my discretion.
...
For this method of game hacking, we care about how variables are stored in memory. We want to find them and edit them. But what happens when the value is being used in a function that we don't know about to do something we can't see?

Sometimes you will run into values that are suspiciously difficult to locate in memory regardless of your expertise at applying differential analysis.
...
It's possible that the times I have discovered a single match, it's a random variable with the same value that has undergone the same value changes. Perhaps somewhere a pixel is changing shades imperceptibly with my edits. Regardless, my karma remains the same.
...
Being able to reverse engineer malware or exploit critical vulnerabilities are some of the most elite skills a hacker can pursue. If script kiddies use other people's tools, and hackers make their own, the reverse engineer is the next step in the evolution.
